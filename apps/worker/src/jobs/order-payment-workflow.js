const paidAt = (event) => new Date(event.metadata.occurredAt);
export const handlePaymentCompletedOrderWorkflow = async (prisma, event) => {
    await prisma.$transaction(async (tx) => {
        ...
        for (const item of order.items) {
            ...
            for (const reservation of activeReservations) {
                ...
                reservationGroups.set(reservation.inventoryItemId, (reservationGroups.get(reservation.inventoryItemId) ?? 0) + reservation.quantity);
            }
